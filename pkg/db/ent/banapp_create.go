...
func (bac *BanAppCreate) Save(ctx context.Context) (*BanApp, error) {
	...
	if len(bac.hooks) == 0 {
		...
		nv, ok := v.(*BanApp)
		if !ok {
			return nil, fmt.Errorf("unexpected node type %T returned from BanAppMutation", v)
		}
		node = nv
	}
	return node, err
...
func (bac *BanAppCreate) defaults() error {
	...
	if _, ok := bac.mutation.EntID(); !ok {
		if banapp.DefaultEntID == nil {
			return fmt.Errorf("ent: uninitialized banapp.DefaultEntID (forgotten import ent/runtime?)")
		}
...
func (bac *BanAppCreate) check() error {
	...
	if _, ok := bac.mutation.EntID(); !ok {
		return &ValidationError{Name: "ent_id", err: errors.New(`ent: missing required field "BanApp.ent_id"`)}
	}
