...
func (otpc *OAuthThirdPartyCreate) Save(ctx context.Context) (*OAuthThirdParty, error) {
	...
	if len(otpc.hooks) == 0 {
		...
		var mut Mutator = MutateFunc(func(ctx context.Context, m Mutation) (Value, error) {
			...
			otpc.mutation = mutation
			if node, err = otpc.sqlSave(ctx); err != nil {
				return nil, err
			}
			mutation.id = &node.ID
			mutation.done = true
		...
		for i := len(otpc.hooks) - 1; i >= 0; i-- {
			if otpc.hooks[i] == nil {
				return nil, fmt.Errorf("ent: uninitialized hook (forgotten import ent/runtime?)")
			}
			mut = otpc.hooks[i](mut)
		}
		v, err := mut.Mutate(ctx, otpc.mutation)
		if err != nil {
			return nil, err
		}
...
func (otpc *OAuthThirdPartyCreate) defaults() error {
	if _, ok := otpc.mutation.CreatedAt(); !ok {
		if oauththirdparty.DefaultCreatedAt == nil {
			return fmt.Errorf("ent: uninitialized oauththirdparty.DefaultCreatedAt (forgotten import ent/runtime?)")
		}
		v := oauththirdparty.DefaultCreatedAt()
		otpc.mutation.SetCreatedAt(v)
	}
	if _, ok := otpc.mutation.UpdatedAt(); !ok {
		if oauththirdparty.DefaultUpdatedAt == nil {
			return fmt.Errorf("ent: uninitialized oauththirdparty.DefaultUpdatedAt (forgotten import ent/runtime?)")
		}
		v := oauththirdparty.DefaultUpdatedAt()
		otpc.mutation.SetUpdatedAt(v)
	}
	if _, ok := otpc.mutation.DeletedAt(); !ok {
		if oauththirdparty.DefaultDeletedAt == nil {
			return fmt.Errorf("ent: uninitialized oauththirdparty.DefaultDeletedAt (forgotten import ent/runtime?)")
		}
...
func (otpc *OAuthThirdPartyCreate) check() error {
	if _, ok := otpc.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "created_at", err: errors.New(`ent: missing required field "OAuthThirdParty.created_at"`)}
	}
	if _, ok := otpc.mutation.UpdatedAt(); !ok {
		return &ValidationError{Name: "updated_at", err: errors.New(`ent: missing required field "OAuthThirdParty.updated_at"`)}
	}
	if _, ok := otpc.mutation.DeletedAt(); !ok {
...
func (u *OAuthThirdPartyUpsertOne) Exec(ctx context.Context) error {
	if len(u.create.conflict) == 0 {
		return errors.New("ent: missing options for OAuthThirdPartyCreate.OnConflict")
	}
...
func (u *OAuthThirdPartyUpsertBulk) Exec(ctx context.Context) error {
	for i, b := range u.create.builders {
		if len(b.conflict) != 0 {
			return fmt.Errorf("ent: OnConflict was set for builder %d. Set it on the OAuthThirdPartyCreateBulk instead", i)
		}
	}
	if len(u.create.conflict) == 0 {
		return errors.New("ent: missing options for OAuthThirdPartyCreateBulk.OnConflict")
	}
	return u.create.Exec(ctx)
}
