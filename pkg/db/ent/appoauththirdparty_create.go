...
func (aotpc *AppOAuthThirdPartyCreate) Save(ctx context.Context) (*AppOAuthThirdParty, error) {
	...
	if err := aotpc.defaults(); err != nil {
		return nil, err
	}
	if len(aotpc.hooks) == 0 {
		if err = aotpc.check(); err != nil {
			return nil, err
		}
		node, err = aotpc.sqlSave(ctx)
	} else {
		var mut Mutator = MutateFunc(func(ctx context.Context, m Mutation) (Value, error) {
			...
			aotpc.mutation = mutation
			if node, err = aotpc.sqlSave(ctx); err != nil {
				return nil, err
			}
			mutation.id = &node.ID
			mutation.done = true
		...
		for i := len(aotpc.hooks) - 1; i >= 0; i-- {
			if aotpc.hooks[i] == nil {
				return nil, fmt.Errorf("ent: uninitialized hook (forgotten import ent/runtime?)")
			}
			mut = aotpc.hooks[i](mut)
		}
		v, err := mut.Mutate(ctx, aotpc.mutation)
		if err != nil {
			return nil, err
		}
...
func (aotpc *AppOAuthThirdPartyCreate) defaults() error {
	if _, ok := aotpc.mutation.CreatedAt(); !ok {
		if appoauththirdparty.DefaultCreatedAt == nil {
			return fmt.Errorf("ent: uninitialized appoauththirdparty.DefaultCreatedAt (forgotten import ent/runtime?)")
		}
		v := appoauththirdparty.DefaultCreatedAt()
		aotpc.mutation.SetCreatedAt(v)
	}
	if _, ok := aotpc.mutation.UpdatedAt(); !ok {
		if appoauththirdparty.DefaultUpdatedAt == nil {
			return fmt.Errorf("ent: uninitialized appoauththirdparty.DefaultUpdatedAt (forgotten import ent/runtime?)")
		}
		v := appoauththirdparty.DefaultUpdatedAt()
		aotpc.mutation.SetUpdatedAt(v)
	}
	if _, ok := aotpc.mutation.DeletedAt(); !ok {
		if appoauththirdparty.DefaultDeletedAt == nil {
			return fmt.Errorf("ent: uninitialized appoauththirdparty.DefaultDeletedAt (forgotten import ent/runtime?)")
		}
	...
	if _, ok := aotpc.mutation.ThirdPartyID(); !ok {
		if appoauththirdparty.DefaultThirdPartyID == nil {
			return fmt.Errorf("ent: uninitialized appoauththirdparty.DefaultThirdPartyID (forgotten import ent/runtime?)")
		}
...
func (aotpc *AppOAuthThirdPartyCreate) check() error {
	if _, ok := aotpc.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "created_at", err: errors.New(`ent: missing required field "AppOAuthThirdParty.created_at"`)}
	}
	if _, ok := aotpc.mutation.UpdatedAt(); !ok {
		return &ValidationError{Name: "updated_at", err: errors.New(`ent: missing required field "AppOAuthThirdParty.updated_at"`)}
	}
	if _, ok := aotpc.mutation.DeletedAt(); !ok {
...
func (u *AppOAuthThirdPartyUpsertOne) Exec(ctx context.Context) error {
	if len(u.create.conflict) == 0 {
		return errors.New("ent: missing options for AppOAuthThirdPartyCreate.OnConflict")
	}
...
func (u *AppOAuthThirdPartyUpsertBulk) Exec(ctx context.Context) error {
	for i, b := range u.create.builders {
		if len(b.conflict) != 0 {
			return fmt.Errorf("ent: OnConflict was set for builder %d. Set it on the AppOAuthThirdPartyCreateBulk instead", i)
		}
	}
	if len(u.create.conflict) == 0 {
		return errors.New("ent: missing options for AppOAuthThirdPartyCreateBulk.OnConflict")
	}
	return u.create.Exec(ctx)
}
