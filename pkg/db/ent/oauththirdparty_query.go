...
func (otpq *OAuthThirdPartyQuery) Order(o ...OrderFunc) *OAuthThirdPartyQuery {
...
func (otpq *OAuthThirdPartyQuery) First(ctx context.Context) (*OAuthThirdParty, error) {
	nodes, err := otpq.Limit(1).All(ctx)
	if err != nil {
		return nil, err
	}
	if len(nodes) == 0 {
...
func (otpq *OAuthThirdPartyQuery) Only(ctx context.Context) (*OAuthThirdParty, error) {
	nodes, err := otpq.Limit(2).All(ctx)
	if err != nil {
		return nil, err
	}
	switch len(nodes) {
...
func (otpq *OAuthThirdPartyQuery) Clone() *OAuthThirdPartyQuery {
	...
	return &OAuthThirdPartyQuery{
		...
		limit:      otpq.limit,
		offset:     otpq.offset,
		order:      append([]OrderFunc{}, otpq.order...),
...
func (otpq *OAuthThirdPartyQuery) GroupBy(field string, fields ...string) *OAuthThirdPartyGroupBy {
	grbuild := &OAuthThirdPartyGroupBy{config: otpq.config}
	grbuild.fields = append([]string{field}, fields...)
	grbuild.path = func(ctx context.Context) (prev *sql.Selector, err error) {
		if err := otpq.prepareQuery(ctx); err != nil {
			return nil, err
		}
		return otpq.sqlQuery(ctx), nil
	}
...
func (otpq *OAuthThirdPartyQuery) Select(fields ...string) *OAuthThirdPartySelect {
	otpq.fields = append(otpq.fields, fields...)
	selbuild := &OAuthThirdPartySelect{OAuthThirdPartyQuery: otpq}
	selbuild.label = oauththirdparty.Label
	selbuild.flds, selbuild.scan = &otpq.fields, selbuild.Scan
	return selbuild
}

func (otpq *OAuthThirdPartyQuery) prepareQuery(ctx context.Context) error {
	for _, f := range otpq.fields {
	...
	if oauththirdparty.Policy == nil {
		return errors.New("ent: uninitialized oauththirdparty.Policy (forgotten import ent/runtime?)")
	}
	if err := oauththirdparty.Policy.EvalQuery(ctx, otpq); err != nil {
		return err
	}
	return nil
}
...
func (otpq *OAuthThirdPartyQuery) sqlCount(ctx context.Context) (int, error) {
	...
	_spec.Node.Columns = otpq.fields
	if len(otpq.fields) > 0 {
		_spec.Unique = otpq.unique != nil && *otpq.unique
	}
	return sqlgraph.CountNodes(ctx, otpq.driver, _spec)
}

func (otpq *OAuthThirdPartyQuery) sqlExist(ctx context.Context) (bool, error) {
	n, err := otpq.sqlCount(ctx)
	if err != nil {
...
func (otpq *OAuthThirdPartyQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if unique := otpq.unique; unique != nil {
		_spec.Unique = *unique
	}
	if fields := otpq.fields; len(fields) > 0 {
	...
	if limit := otpq.limit; limit != nil {
		_spec.Limit = *limit
	}
	if offset := otpq.offset; offset != nil {
		_spec.Offset = *offset
	}
...
func (otpq *OAuthThirdPartyQuery) sqlQuery(ctx context.Context) *sql.Selector {
	...
	if otpq.unique != nil && *otpq.unique {
		selector.Distinct()
	}
...
func (otpgb *OAuthThirdPartyGroupBy) Scan(ctx context.Context, v interface{}) error {
	query, err := otpgb.path(ctx)
	if err != nil {
		return err
	}
	otpgb.sql = query
	return otpgb.sqlScan(ctx, v)
}

func (otpgb *OAuthThirdPartyGroupBy) sqlScan(ctx context.Context, v interface{}) error {
	for _, f := range otpgb.fields {
		if !oauththirdparty.ValidColumn(f) {
			return &ValidationError{Name: f, err: fmt.Errorf("invalid field %q for group-by", f)}
		}
	}
	selector := otpgb.sqlQuery()
	if err := selector.Err(); err != nil {
		return err
	}
	rows := &sql.Rows{}
	query, args := selector.Query()
	if err := otpgb.driver.Query(ctx, query, args, rows); err != nil {
		return err
	}
	defer rows.Close()
...
func (otpgb *OAuthThirdPartyGroupBy) sqlQuery() *sql.Selector {
	selector := otpgb.sql.Select()
	...
	if len(selector.SelectedColumns()) == 0 {
		columns := make([]string, 0, len(otpgb.fields)+len(otpgb.fns))
		for _, f := range otpgb.fields {
	...
	return selector.GroupBy(selector.Columns(otpgb.fields...)...)
}
...
func (otps *OAuthThirdPartySelect) Scan(ctx context.Context, v interface{}) error {
	if err := otps.prepareQuery(ctx); err != nil {
		return err
	}
	otps.sql = otps.OAuthThirdPartyQuery.sqlQuery(ctx)
	return otps.sqlScan(ctx, v)
}

func (otps *OAuthThirdPartySelect) sqlScan(ctx context.Context, v interface{}) error {
	rows := &sql.Rows{}
	query, args := otps.sql.Query()
