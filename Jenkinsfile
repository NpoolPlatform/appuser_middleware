    ...
    stage('Deploy for testing') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          ...
          sed -i "s/replicas: 2/replicas: $REPLICAS_COUNT/g" cmd/appuser-middleware/k8s/02-appuser-middleware.yaml
          sed -i "s/imagePullPolicy: Always/imagePullPolicy: IfNotPresent/g" cmd/appuser-middleware/k8s/02-appuser-middleware.yaml
          make deploy-to-k8s-cluster
        '''.stripIndent())
      }
    }
